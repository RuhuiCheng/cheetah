...
def node_init(dag, ls_node):
    ls_dm_node = []
    ls_node = [item for item in ls_node if item[1] == etl_layer.dws.name]
    for item in ls_node:
        flow_tag = item[2]
...
def per_node_init(dag, flow_tag):
    ...
    ops_data = all_data[etl_layer.dws.name]['ops']
    for item in ops_data:
        if item['flow_tag'] == flow_tag:
            ...
            task_id='{0}_{1}'.format(etl_layer.dws.name, task_item['task_name']),
...
def relation_init(ls_relation, ls_dws_node, ls_dm_node):
    for dws_item in ls_dws_node:
        ...
        for dm_item in ls_dm_node:
            ...
            rel_dm = [item for item in ls_relation if item[0] == dws_node_tag and item[1] == dm_node_tag]
            if len(rel_dm) == 1:
                dws_node >> dm_node
...
def ops_call_back(ds, **kwargs):
    ...
    logger.info("dws task start -->{0}".format(msg))
    jdbc_ops.execute_hql(conn_id.conn_hive_jdbc.name, sql_text)
    logger.info("dws task done -->{0}".format(msg))
