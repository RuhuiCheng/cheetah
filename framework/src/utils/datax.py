...
def run(json_path):
    ...
    options, args = parser.parse_args([json_path])
    db = BaseHook.get_connection(conn_id.conn_hive_jdbc.name)
    __user = db.login
    __user_dir = "{0}/conf".format(DATAX_HOME)
    options.params ="-DHADOOP_USER_NAME={0}".format(__user)
    options.params =options.params + " " + "-Duser.dir={0}".format(__user_dir)
